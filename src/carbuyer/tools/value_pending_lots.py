"""CLI helper for the value-pending-lots skill.
...
Three subcommands, each safe to run repeatedly:
  ...
  list-groups       Emit JSON describing (year, make, model) groups whose lots
                    ...
                    the candidate mileage range and trim variety so the skill
...
The skill in .claude/skills/value-pending-lots/SKILL.md orchestrates these
via Bash. Business logic (what to research, when to give up) lives in the
skill prompt; data plumbing lives here.
"""
