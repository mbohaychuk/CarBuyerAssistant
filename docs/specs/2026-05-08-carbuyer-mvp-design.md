...
- **OpenAI API over Max plan:** Anthropic's TOS explicitly disclaims SDK use of Max-plan auth; Claude Agent SDK doesn't actually support Max OAuth (issues #559, #11). Direct API on gpt-4o-mini costs ~$12/mo at MVP volume vs $100/mo Max plan tier, and the provider abstraction lets us swap to Anthropic/Ollama later.
