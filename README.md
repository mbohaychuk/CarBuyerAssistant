...
```
 ...
 └────────────┘    └────────────┘         └────────────┘     └────────────┘
                          ...
                          └──────────────────┘
                          ┌──────────────────┐
                          │ source_watchdog  │     hourly: Discord alert when
                          │ (hourly)         │     a source stops producing
                          └──────────────────┘
...
Eleven systemd units: a Postgres container wrapper, six continuous workers (`Restart=always` — bot, dashboard, enricher, valuator, notifier, bid_poller), and four timer-driven jobs (ingester, vision, distiller, source_watchdog). Logs go to journald.
...
| Async runtime | Python 3.14, asyncio | LLM calls and HTTP scraping are both IO-bound. |
...
| LLM | OpenAI `gpt-5-nano` — one model, text + vision | Cost target: under $20/month; `reasoning_effort=low` keeps it there. |
...
uv run python -m carbuyer.apps.source_watchdog
