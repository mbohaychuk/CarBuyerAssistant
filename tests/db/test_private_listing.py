...
async def test_upsert_insert_sets_original_and_zero_drop_count(session: AsyncSession) -> None:
    listing = await upsert_private_listing(session, _raw_listing(asking="18000"), parser_version="v1")
    await session.flush()
...
async def test_price_drop_increments_count_and_keeps_original(session: AsyncSession) -> None:
    await upsert_private_listing(session, _raw_listing(asking="18000"), parser_version="v1")
    await session.flush()
    listing = await upsert_private_listing(session, _raw_listing(asking="15000"), parser_version="v1")
    await session.flush()
    ...
    listing = await upsert_private_listing(session, _raw_listing(asking="14000"), parser_version="v1")
    await session.flush()
...
async def test_price_increase_does_not_increment_drop_count(session: AsyncSession) -> None:
    await upsert_private_listing(session, _raw_listing(asking="15000"), parser_version="v1")
    await session.flush()
    listing = await upsert_private_listing(session, _raw_listing(asking="16000"), parser_version="v1")
    await session.flush()
    assert listing.price_drop_count == 0
